...
class KegiatanController extends \App\Http\Controllers\Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $opsiFormulir = $request->input('opsi_formulir') === '1';
        ...
        $kegiatan = Kegiatan::create([
            ...
            'tampilkan'     => $request->input('tampilkan') === '1',
            'presensi'      => $request->input('presensi') === '1',
            'pendaftaran'   => $request->input('pendaftaran'),
            'opsi_formulir' => $opsiFormulir,
        ]);

        if ($opsiFormulir && $request->has('pertanyaans')) {
            foreach ((array) $request->input('pertanyaans') as $p) {
                if (empty($p['pertanyaan'])) continue;
                $kegiatan->pertanyaans()->create([
                    'pertanyaan' => $p['pertanyaan'],
                    'tipe'       => $p['tipe'] ?? 'text',
                    'wajib'      => ($p['wajib'] ?? 'false') === 'true',
                    'opsi'       => !empty($p['opsi']) ? array_values(array_filter($p['opsi'])) : null,
                ]);
            }
        }
    ...
    public function update(Request $request, Kegiatan $kegiatan)
    {
        ...
        if ($opsiFormulir && $request->has('pertanyaans')) {
            ...
            foreach ((array) $request->input('pertanyaans') as $p) {
                ...
                if (!empty($p['id'])) {
                    $existing = $kegiatan->pertanyaans()->find($p['id']);
                    if ($existing) { $existing->update($data); $submittedIds[] = $p['id']; }
                } else {
                    $new = $kegiatan->pertanyaans()->create($data);
                    $submittedIds[] = $new->id;
                }
            }
            $kegiatan->pertanyaans()->whereNotIn('id', $submittedIds)->delete();
        } else {
