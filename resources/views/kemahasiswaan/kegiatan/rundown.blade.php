    ...
    <div id="modal" class="fixed inset-0 z-[9999]" style="display: none;">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md relative">
                ...
                <form id="rundownForm" method="POST" action="{{ route('kemahasiswaan.kegiatan.rundown.store', $kegiatan) }}"
                    class="p-6 space-y-4">
                    @csrf
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const baseUrl = "{{ route('kemahasiswaan.kegiatan.rundown', $kegiatan) }}";
            const storeUrl = "{{ route('kemahasiswaan.kegiatan.rundown.store', $kegiatan) }}";
