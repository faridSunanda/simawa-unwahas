...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <!-- Add Button - Pojok Kanan Atas -->
        <button onclick="openFormModal()"
            class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-simawa-500 to-simawa-700 text-white rounded-lg hover:from-simawa-600 hover:to-simawa-800 transition-all shadow-lg shadow-simawa-500/25 text-sm font-medium">
    ...
    <!-- Forms List by Year (Accordion Style) -->
                    ...
                    <button onclick="toggleAccordion('tahun-{{ $thn }}')" 
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="text-left">
                                ...
                                <p class="text-sm text-white/80">{{ $count }} Formulir</p>
                            </div>
                        </div>
                    ...
                    <div id="tahun-{{ $thn }}" class="hidden border-t border-gray-100">
                                        ...
                                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                                            <div class="flex-1 min-w-0">
                                                <div class="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                                                    <h5 class="font-semibold text-gray-800">{{ $formulir->judul }}</h5>
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700 w-fit">
                                                        {{ $formulir->kategoriPrestasi->nama }}
                                                    </span>
                                                    @if($formulir->is_active)
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700 w-fit">
                                                            Aktif
                                                        </span>
                                                    @else
                                            ...
                                            <div class="flex items-center gap-2 flex-shrink-0">
                                                <button onclick="viewFormPreview('{{ $formulir->id }}')"
                                                    class="flex h-8 w-8 items-center justify-center text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors"
                                                    title="Lihat Detail Formulir">
                                                    <ion-icon name="document-text-outline" class="text-base"></ion-icon>
                                                </button>
                                                <button onclick="toggleVisibility('{{ $formulir->id }}', {{ $formulir->is_active ? 'true' : 'false' }})"
                                                    class="flex h-8 w-8 items-center justify-center rounded-lg transition-colors {{ $formulir->is_active ? 'bg-green-100 text-green-600 hover:bg-green-200' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}"
                                                    title="{{ $formulir->is_active ? 'Sembunyikan Formulir' : 'Tampilkan Formulir' }}">
                                                    <ion-icon name="{{ $formulir->is_active ? 'eye-outline' : 'eye-off-outline' }}" class="text-base"></ion-icon>
                                                </button>
                                                ...
                                                <button onclick="deleteForm('{{ $formulir->id }}')"
                                                    class="flex h-8 w-8 items-center justify-center text-white bg-red-500 hover:bg-red-600 rounded-lg transition-colors"
                                                    title="Hapus">
                        ...
                        @else
                            <div class="p-8 text-center">
                                ...
                                <p class="text-gray-500 text-sm">Tidak ada formulir di tahun ini</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="p-4 sm:p-6 space-y-4 sm:space-y-6 max-h-[60vh] overflow-y-auto">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select name="tahun" id="formTahun" required
                                    class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-simawa-500/20 focus:border-simawa-500 transition-all text-sm">
                                    @php
                                        $currentYear = date('Y');
    ...
    <div id="toggleConfirmModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" onclick="closeToggleConfirmModal()"></div>
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm transform transition-all">
                <div class="p-6 text-center">
                    <div class="w-16 h-16 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-4">
                        <ion-icon name="eye-outline" id="toggleIcon" class="text-3xl text-yellow-600"></ion-icon>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-800 mb-2">Ubah Status Tampilan?</h3>
                    <p id="toggleMessage" class="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin mengubah status visibility formulir ini?</p>
                    <div class="flex gap-3">
                        <button type="button" onclick="closeToggleConfirmModal()"
                            class="flex-1 px-4 py-2.5 border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-all text-sm font-medium">
                            Batal
                        </button>
                        <button type="button" onclick="confirmToggleVisibility()"
                            class="flex-1 px-4 py-2.5 bg-gradient-to-r from-yellow-500 to-yellow-600 text-white rounded-lg hover:from-yellow-600 hover:to-yellow-700 transition-all text-sm font-medium">
                            Ya, Ubah
                        </button>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function toggleVisibility(id, isActive) {
            currentToggleId = id;
            ...
            if (isActive) {
                ...
                message.textContent = 'Formulir ini akan disembunyikan dari mahasiswa. Lanjutkan?';
            } else {
                icon.setAttribute('name', 'eye-outline');
                icon.classList.remove('text-red-600');
                icon.classList.add('text-green-600');
                message.textContent = 'Formulir ini akan ditampilkan ke mahasiswa. Lanjutkan?';
            }
        ...
        function confirmToggleVisibility() {
            if (currentToggleId) {
                document.getElementById('toggleForm').action = "{{ url('superadmin/formulir-prestasi') }}/" + currentToggleId + "/toggle";
