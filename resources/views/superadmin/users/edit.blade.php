...
@section('page-description', 'Perbarui data pengguna')
...
@section('content')
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500 ml-7">Perbarui data pengguna: {{ $user->name }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
        <form action="{{ route('superadmin.users.update', $user) }}" method="POST" id="userForm">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <select name="role_id" id="role_id" required onchange="toggleRoleFields()"
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" data-role="{{ $role->name }}" {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }}>
                                {{ $role->display_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="mahasiswaFields" class="hidden space-y-5 p-4 bg-blue-50 rounded-lg border border-blue-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="nim" class="block text-sm font-medium text-gray-700 mb-1.5">NIM <span class="text-red-500">*</span></label>
                            <input type="text" name="nim" id="nim" value="{{ old('nim', $user->mahasiswa?->nim) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="angkatan" id="angkatan" value="{{ old('angkatan', $user->mahasiswa?->angkatan) }}"
                ...
                <div id="dosenFields" class="hidden space-y-5 p-4 bg-purple-50 rounded-lg border border-purple-100">
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $user->dosen?->nip) }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-simawa-500 focus:border-simawa-500 transition-colors"
                ...
                <div id="staffFields" class="hidden space-y-5 p-4 bg-amber-50 rounded-lg border border-amber-100">
                    ...
                    <div>
                        ...
                        <input type="text" name="bagian" id="bagian" value="{{ old('bagian', $user->staff?->bagian ?? 'Kemahasiswaan') }}"
                            class="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-simawa-500 focus:border-simawa-500 transition-colors"
                ...
                <div id="unitFields" class="hidden space-y-5 p-4 bg-green-50 rounded-lg border border-green-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="fakultas" class="block text-sm font-medium text-gray-700 mb-1.5">Fakultas <span class="text-red-500">*</span></label>
                            <input type="text" name="fakultas" id="fakultas" value="{{ old('fakultas', $user->mahasiswa?->fakultas ?? $user->dosen?->fakultas) }}" list="fakultasList"
                        ...
                        <div id="prodiContainer">
                            <label for="prodi" class="block text-sm font-medium text-gray-700 mb-1.5">Program Studi <span id="prodiRequired" class="text-red-500 hidden">*</span></label>
                            <input type="text" name="prodi" id="prodi" value="{{ old('prodi', $user->mahasiswa?->prodi ?? $user->dosen?->prodi) }}" list="prodiList"
